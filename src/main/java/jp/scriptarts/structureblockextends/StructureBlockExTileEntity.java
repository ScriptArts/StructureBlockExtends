...
public class StructureBlockExTileEntity extends StructureBlockTileEntity {
    ...
    public void func_230337_a_(BlockState p_230337_1_, CompoundNBT p_230337_2_) {
        ...
        int i = MathHelper.clamp(p_230337_2_.getInt("posX"), SIZE_MIN, SIZE_MAX);
        int j = MathHelper.clamp(p_230337_2_.getInt("posY"), SIZE_MIN, SIZE_MAX);
        int k = MathHelper.clamp(p_230337_2_.getInt("posZ"), SIZE_MIN, SIZE_MAX);
        this.position = new BlockPos(i, j, k);
        int l = MathHelper.clamp(p_230337_2_.getInt("sizeX"), 0, SIZE_MAX);
        int i1 = MathHelper.clamp(p_230337_2_.getInt("sizeY"), 0, SIZE_MAX);
        int j1 = MathHelper.clamp(p_230337_2_.getInt("sizeZ"), 0, SIZE_MAX);
    ...
    public boolean detectSize() {
        if (this.mode != StructureMode.SAVE) {
            ...
            List<StructureBlockExTileEntity> list = this.getNearbyCornerBlocks(blockpos1, blockpos2);
            List<StructureBlockExTileEntity> list1 = this.filterRelatedCornerBlocks(list);
            if (list1.size() < 1) {
                return false;
            } else {
    ...
    private List<StructureBlockExTileEntity> filterRelatedCornerBlocks(List<StructureBlockExTileEntity> p_184415_1_) {
        Predicate<StructureBlockExTileEntity> predicate = (p_200665_1_) -> {
    ...
    private List<StructureBlockExTileEntity> getNearbyCornerBlocks(BlockPos p_184418_1_, BlockPos p_184418_2_) {
        List<StructureBlockExTileEntity> list = Lists.newArrayList();

        for(BlockPos blockpos : BlockPos.getAllInBoxMutable(p_184418_1_, p_184418_2_)) {
            BlockState blockstate = this.world.getBlockState(blockpos);
            if (blockstate.isIn(Blocks.STRUCTURE_BLOCK)) {
                TileEntity tileentity = this.world.getTileEntity(blockpos);
                if (tileentity != null && tileentity instanceof StructureBlockExTileEntity) {
                    list.add((StructureBlockExTileEntity)tileentity);
                }
            }
        }
    ...
    private MutableBoundingBox calculateEnclosingBoundingBox(BlockPos p_184416_1_, List<StructureBlockExTileEntity> p_184416_2_) {
        ...
        for(StructureBlockExTileEntity StructureBlockExTileEntity : p_184416_2_) {
            BlockPos blockpos1 = StructureBlockExTileEntity.getPos();
